-- View: ptc.trip_data_routing_20161020_ext

DROP MATERIALIZED VIEW ptc.trip_data_routing_20161020_ext;

CREATE MATERIALIZED VIEW ptc.trip_data_routing_20161020_ext
...
AS
 WITH trips AS (
         SELECT trip_data_clean.mlsassignedptc,
            ...
            COALESCE(origin_gateway.source::numeric, origin_nodes.source) AS source,
            trip_data_clean.dropoff_mun_id,
            trip_data_clean.dropoff_gc_intersection_id,
            trip_data_clean.dropoff_gc_centreline_id,
            COALESCE(dest_gateway.target::numeric, dest_nodes.target) AS target,
             ...
             LEFT JOIN LATERAL ( 
				 SELECT dt_gate.node_id AS target
				  ...
				  WHERE trip_data_clean.dropoff_mun_id = dt_gate.mun_id AND (bound is NULL OR bound = 'outbound')
					--Origins close to gateway should get matched to all gateways
				AND ((st_dwithin(st_transform(origint.geom, 98012), st_transform(dt_gate.geom, 98012), 3000::double precision) AND dt_gate.short_long = 'shorter_points')
					 --Origins further away from gateways should get matched to only highway gateways
					 OR (NOT st_dwithin(st_transform(origint.geom, 98012), st_transform(dt_gate.geom, 98012), 3000::double precision) AND dt_gate.short_long = 'longer_points'))
			 ...
			 LEFT JOIN LATERAL ( 
				 SELECT og_gate.node_id AS source
				  ...
				  WHERE trip_data_clean.pickup_mun_id = og_gate.mun_id AND (bound is NULL OR bound = 'inbound')
								--Destinations close to gateway should get matched to all gateways
								AND ((st_dwithin(st_transform(destinint.geom, 98012), st_transform(og_gate.geom, 98012), 3000::double precision) AND og_gate.short_long = 'shorter_points') 
								--Destinations further away from gateways should get matched to only highway gateways	 
									 OR (NOT st_dwithin(st_transform(destinint.geom, 98012), st_transform(og_gate.geom, 98012), 3000::double precision) AND og_gate.short_long = 'longer_points'))
         ...
         SELECT reordered_stops.mlsassignedptc,
          ...
          WHERE reordered_stops.dropoff_mun_id IN (61,38,71,57,8,41) OR reordered_stops.pickup_mun_id IN (61,38,71,57,8,41) 
        )
 SELECT trips.mlsassignedptc,
  ...
  WHERE pooled.ptctripid IS NULL AND (trips.dropoff_mun_id IN (61,38,71,57,8,41) OR trips.pickup_mun_id IN (61,38,71,57,8,41))
UNION ALL
...
ALTER TABLE ptc.trip_data_routing_20161020_ext
    OWNER TO rdumas;

COMMENT ON MATERIALIZED VIEW ptc.trip_data_routing_20161020_ext
    IS 'Representative day of PTC data for Oct ''16, with nodes routable on the routing_streets_18_3b network and pooled trip stops reordered. Only trips outside the city';

GRANT ALL ON TABLE ptc.trip_data_routing_20161020_ext TO rdumas;
GRANT SELECT ON TABLE ptc.trip_data_routing_20161020_ext TO bdit_humans;

CREATE INDEX trip_data_routing_20161020_ext_ptctripid_trippassengerid_idx
    ON ptc.trip_data_routing_20161020_ext USING btree
